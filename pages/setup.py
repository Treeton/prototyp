...
# input file
INPUT_FILE = 'database/datamodel_Agathon_20210908.xlsx'
...
def run():
    ...
    with col2:
        ...
        time.sleep(0.5)
        st.session_state['machines_all'] = file_input.get_data(INPUT_FILE, MACHINES_SHEET)
        progress_bar.progress(25)

        # load options
        progress_text.markdown(_('...options'))
        time.sleep(0.5)
        st.session_state['options_all'] = file_input.get_data(INPUT_FILE, OPTIONS_SHEET)
        progress_bar.progress(50)

        # load production models
        progress_text.markdown(_('...production models'))
        time.sleep(0.5)
        st.session_state['production_all'] = file_input.get_data(INPUT_FILE, PRODUCTION_SHEET)
        progress_bar.progress(75)

        # load payment models
        progress_text.markdown(_('...payment models'))
        time.sleep(0.5)
        st.session_state['payment_all'] = file_input.get_data(INPUT_FILE, PAYMENT_SHEET)
