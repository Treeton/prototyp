...
def show():
    ...
    # styling
    css = file_input.get_css('css/simulator.css')
    ...
    with col2:
        ...
        upfront_payment = st.slider(_('Upfront payment [CHF]'), 0, st.session_state['machine']['costs'], 220000,
                                    step=10000)
...
def get_compatible_models(models) -> pd.DataFrame:
    ...
    if 'machine' in st.session_state:
    ...
    compatible = models.loc[models['Compatible Machines'].str.contains(machine['Id'])]
    ...
    # filter by options
    # TODO: filter models by selected options
...
def compute_classic_payment(n_months, upfront, rates, amortization, maintenance, costs):
    ...
    for i in range(n_months):
        rate = 0
        if costs > 0:
            rate = (costs-upfront)*rates + amortization
            if i >= 24:
                rate += maintenance
            costs -= amortization
